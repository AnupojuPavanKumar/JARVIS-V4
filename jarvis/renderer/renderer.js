...
Think step-by-step before answering. Prioritize logic, absolute accuracy, and concise code formatting. Do not hallucinate. Respond in extremely short, punchy sentences. DO NOT use markdown lists or bullet points unless explicitly requested. Maximize brevity to optimize voice output.`, window.ToolExecutor && window.Skills && window.Skills._loaded && window.Skills._manifest.length && (t += `
...
`).forEach(o => o && termPrint("err", o)), s.error && !s.stdout && !s.stderr && termPrint("err", s.error), termPrint("info", `[exit ${s.exitCode ?? 0}]`)
  ...
  </div>`} function initOrbCanvas() { const e = document.getElementById("jarvis-orb-canvas"); if (!e) return null; const t = e.getContext("2d"), n = e.width, s = e.height; let o = null, i = 0; function a() { return getComputedStyle(document.documentElement).getPropertyValue("--cyan").trim() || "#00d4ff" } function c(g) { const d = g.match(/^#([0-9a-f]{6})$/i); if (d) { const m = parseInt(d[1], 16); return [m >> 16 & 255, m >> 8 & 255, m & 255] } const u = g.match(/rgb\((\d+),\s*(\d+),\s*(\d+)\)/); return u ? [+u[1], +u[2], +u[3]] : [0, 212, 255] } const l = Array.from({ length: 40 }, () => ({ angle: Math.random() * Math.PI * 2, radius: 80 + Math.random() * 80, speed: (Math.random() - .5) * .008, size: Math.random() * 2.5 + .5, opacity: Math.random() * .6 + .2, phase: Math.random() * Math.PI * 2 })); function p() { t.clearRect(0, 0, n, s); const g = a(), [d, u, m] = c(g), v = n / 2, h = s / 2, w = state.isStreaming, x = w ? 3.5 : 1, b = w ? 1.8 : 1; i += .012 * x; const S = t.createRadialGradient(v, h, 10, v, h, 190); S.addColorStop(0, `rgba(${d},${u},${m},${.22 * b})`), S.addColorStop(.4, `rgba(${d},${u},${m},${.06 * b})`), S.addColorStop(1, `rgba(${d},${u},${m},0)`), t.beginPath(), t.arc(v, h, 190, 0, Math.PI * 2), t.fillStyle = S, t.fill(), [{ radius: 145, dash: [16, 10], width: 1.2, opacity: .25, dir: 1 }, { radius: 118, dash: [8, 14], width: 1.5, opacity: .4, dir: -1 }, { radius: 90, dash: [4, 8], width: 1.8, opacity: .55, dir: 1 }, { radius: 62, dash: [2, 6], width: 2, opacity: .7, dir: -1 }].forEach((f, y) => { t.save(), t.translate(v, h), t.rotate(i * (.4 + y * .15) * f.dir), t.beginPath(), t.arc(0, 0, f.radius, 0, Math.PI * 2), t.setLineDash(f.dash), t.strokeStyle = `rgba(${d},${u},${m},${f.opacity * b})`, t.lineWidth = f.width, t.stroke(), t.restore() }); const B = 8; for (let f = 0; f < B; f++) { const y = f / B * Math.PI * 2 + i * .5, T = v + Math.cos(y) * 145, A = h + Math.sin(y) * 145, C = .5 + .5 * Math.sin(i * 3 + f * .8); t.beginPath(), t.arc(T, A, 3 + C * 1.5, 0, Math.PI * 2), t.fillStyle = `rgba(${d},${u},${m},${.5 + C * .5})`, t.fill() } const k = .5 + .5 * Math.sin(i * 1.5), I = 38 + k * (w ? 8 : 4), E = t.createRadialGradient(v - 8, h - 8, 2, v, h, I); E.addColorStop(0, `rgba(255,255,255,${.9 * b})`), E.addColorStop(.3, `rgba(${d},${u},${m},${.85})`), E.addColorStop(.7, `rgba(${d},${u},${m},${.5})`), E.addColorStop(1, `rgba(${d},${u},${m},0.05)`), t.beginPath(), t.arc(v, h, I, 0, Math.PI * 2), t.fillStyle = E, t.fill(); const $ = t.createRadialGradient(v, h, 0, v, h, I * 2.2); if ($.addColorStop(0, `rgba(${d},${u},${m},${.3 * b})`), $.addColorStop(1, `rgba(${d},${u},${m},0)`), t.beginPath(), t.arc(v, h, I * 2.2, 0, Math.PI * 2), t.fillStyle = $, t.fill(), t.save(), t.translate(v, h), t.rotate(i * .3), t.font = `${22 + k * 3}px "Segoe UI Symbol", sans-serif`, t.fillStyle = `rgba(255,255,255,${.85 + k * .15})`, t.textAlign = "center", t.textBaseline = "middle", t.fillText("\u2B21", 0, 0), t.restore(), l.forEach(f => { f.angle += f.speed * x; const y = f.radius + Math.sin(i * .8 + f.phase) * 12, T = v + Math.cos(f.angle) * y, A = h + Math.sin(f.angle) * y, C = f.opacity * (.6 + .4 * Math.sin(i * 1.2 + f.phase)); t.beginPath(), t.arc(T, A, f.size, 0, Math.PI * 2), t.fillStyle = `rgba(${d},${u},${m},${C})`, t.fill() }), w) { const f = i * 2; t.save(), t.translate(v, h), t.rotate(f); const y = t.createLinearGradient(0, 0, 160, 0); y.addColorStop(0, `rgba(${d},${u},${m},0.5)`), y.addColorStop(1, `rgba(${d},${u},${m},0)`), t.beginPath(), t.moveTo(0, 0), t.arc(0, 0, 160, -.05, .05), t.closePath(), t.fillStyle = y, t.fill(), t.restore() } state.isStreaming ? o = requestAnimationFrame(p) : !state.isFocused && !state.isStreaming ? o = setTimeout(() => requestAnimationFrame(p), 500) : o = setTimeout(() => requestAnimationFrame(p), 50) } return p(), () => { cancelAnimationFrame(o), clearTimeout(o) } } function dashCode() {
