...
`); l = d.pop(); for (const u of d) if (u.trim()) try { const m = JSON.parse(u); if (m.message?.content && (r += m.message.content, t(m.message.content, r)), m.done) { n(r); return } } catch { }
  ...
  const _now = new Date();
  t += `\n[CURRENT SYSTEM TIME: ${_now.toLocaleDateString("en-GB", { weekday: "long", day: "2-digit", month: "long", year: "numeric" })}, ${_now.toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}]\n`;
  if (state.systemSpecs && (t += `[HOST HARDWARE SPECIFICATIONS \u2014 use these exact specs if the user asks about the host system performance, CPU, GPU, memory, or hardware details]
...
Root: ${o}File tree:
${a}${r}
[END PROJECT CONTEXT]`} state.webSearchContext && (t += state.webSearchContext.slice(0, _BUDGET_WEB), state.webSearchContext = null);
  if (!state.deepWorkEnabled) {
    t += `[DIRECTIVE]
QUICK CHAT MODE: Keep your response extremely concise, conversational, and direct. Do NOT output step-by-step reasoning, thinking tags, or long explanations. Respond with maximum brevity. Keep answers to 1-2 short sentences. Optimize for quick voice output.`;
  } else {
    t += `[DIRECTIVE]
Think step-by-step before answering. Prioritize logic, absolute accuracy, and concise code formatting. Do not hallucinate. Respond with maximum brevity. Keep answers to 1-2 short sentences unless the user explicitly asks for details. DO NOT use markdown lists or bullet points unless explicitly requested. Optimize for quick voice output.`;
  }
  window.ToolExecutor && window.Skills && window.Skills._loaded && window.Skills._manifest.length && (t += `
...
[... context truncated ...]`); const n = [{ role: "system", content: t }]; if (e?.messages?.length) { const s = state.contextWindow > 0 ? Math.max(0, e.messages.length - state.contextWindow) : 0; e.messages.slice(s).forEach(o => n.push({ role: o.role, content: o.content })) } return n
  ...
  while (true) {
    const match = clean.match(/\{[^{]*?"tool"\s*:/);
    if (!match) break;
    ...
    if (endIdx !== -1) {
      clean = clean.slice(0, idx) + clean.slice(endIdx + 1);
    } else {
      break;
    }
  }
  ...
  const text = (isString ? e : $input.value).trim();
  if (!text || state.isStreaming) return;
  
  if (!state.ollamaOnline && !await checkOllama()) {
    showToast("Ollama is offline. Start Ollama and try again.", "error");
    return;
  }
  
  const timestamp = new Date().toISOString();
  state.conversations[state.mode].messages.push({ role: "user", content: text, timestamp });
  appendUserMessage(text, timestamp);

  if (!isString || e === $input.value) {
    $input.value = "";
    autoResizeInput();
  }
  ...
  try {
    if (state.deepWorkEnabled && window.TaskController) {
      const task = window.TaskController.createTask(text);
      await window.TaskController.executeTask();
    } else {
      const c = createStreamingMessage();
      const payload = typeof buildMessages === "function" ? buildMessages() : state.conversations[state.mode].messages;
      await streamOllama(payload, {
        onChunk: (r, l) => updateStreamingEl(c, l),
        onDone: r => {
          finalizeStreamingEl(c, r);
          state.conversations[state.mode].messages.push({ role: "assistant", content: r, timestamp: new Date().toISOString() });
          if (state.ttsEnabled) {
            const l = extractPlainText(r);
            if (l) speakText(l, !0);
          }
        },
        onError: err => {
          try { c.remove() } catch {}
          appendJarvisMessage(`[ERROR] ${err.message}`, new Date().toISOString());
        }
      });
    }
  } finally {
...
Do NOT say "How can I help", "I am ready", or anything generic. Make it feel alive and contextual.`, c = createStreamingMessage(); await streamOllama([{ role: "user", content: a }], { onChunk: (r, l) => updateStreamingEl(c, l), onDone: r => { if (!r || !r.trim()) { try { c.remove() } catch {} return } finalizeStreamingEl(c, r); state.conversations[state.mode].messages.push({ role: "assistant", content: r, timestamp: new Date().toISOString() }); if (state.ttsEnabled) { const l = extractPlainText(r); l && speakText(l, !0) } }, onError: () => { try { c.remove() } catch {} } })
  ...
  const n = document.createElement("div"); n.className = "message user-message", n.innerHTML = `
    <div class="msg-body">
      <div class="msg-meta"><span class="msg-sender">YOU</span><span class="msg-time">${window.formatTime(t)}</span></div>
  ...
  const n = MODES[state.mode], s = document.createElement("div"); s.className = "message jarvis-message", s.innerHTML = `
    ...
    <div class="msg-body">
      <div class="msg-meta">
        ...
        <span class="msg-time">${window.formatTime(t)}</span>
      </div>
  ...
  function renderMarkdown(e) {
    ...
    let html = "";
    try {
      html = typeof marked !== "undefined" ? marked.parse(e) : `<p>${escHtml(e)}</p>`;
    } catch {
      html = `<p>${escHtml(e)}</p>`;
    }
