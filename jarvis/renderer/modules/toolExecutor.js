...
const ToolExecutor = (() => {
  ...
  function detectToolCall(text) {
    ...
    // Fallback: search string for raw JSON object containing "tool"
    try {
      const idx = text.lastIndexOf('{"tool"');
      if (idx !== -1) {
        let jsonStr = text.slice(idx);
        while (jsonStr.length > 5) {
          try {
            const parsed = JSON.parse(jsonStr);
            if (parsed && typeof parsed.tool === 'string') return parsed;
          } catch (_) {
            jsonStr = jsonStr.slice(0, jsonStr.lastIndexOf('}'));
            if (jsonStr.lastIndexOf('}') === -1) break;
    ...
    try {
      ...
      if (res && res.ok) {
        ...
        if (output.length > 3000) {
          output = output.substring(0, 3000) + `\n\n...[TRUNCATED: Output exceeded 3000 chars. Use other tools to narrow down the search.]`;
        }
