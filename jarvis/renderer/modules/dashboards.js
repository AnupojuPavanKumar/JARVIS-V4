...
function initOrbCanvas() {
  ...
  function draw(timestamp) {
    ...
    const gradKey = `${r},${g},${b},${streaming}`;
    if (cachedGradKey !== gradKey || !cachedGrad) {
      cachedGrad = ctx.createRadialGradient(cx, cy, 10, cx, cy, 190);
      cachedGrad.addColorStop(0, `rgba(${r},${g},${b},${0.22 * glowMult})`);
      cachedGrad.addColorStop(0.4, `rgba(${r},${g},${b},${0.06 * glowMult})`);
      cachedGrad.addColorStop(1, `rgba(${r},${g},${b},0)`);
    ...
    // Optimized Core: using cached gradient with scaling
    ctx.save();
    ctx.translate(cx, cy);
    const scaleFactor = coreR / 46; // normalize to max core radius ~46
    ctx.scale(scaleFactor, scaleFactor);
    
    const coreGrad = ctx.createRadialGradient(-8/scaleFactor, -8/scaleFactor, 2/scaleFactor, 0, 0, 46);
    ...
    const bloomGrad = ctx.createRadialGradient(0, 0, 0, 0, 0, 46 * 2.2);
    bloomGrad.addColorStop(0, `rgba(${r},${g},${b},${0.3 * glowMult})`);
    bloomGrad.addColorStop(1, `rgba(${r},${g},${b},0)`);
    ctx.beginPath();
    ctx.arc(0, 0, 46 * 2.2, 0, Math.PI * 2);
